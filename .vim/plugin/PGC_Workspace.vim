...
function! PGC_Workspace#Save_Workspace ( ... )
    if (a:0 == 0)
        echomsg "Empty args"
        return 
    endif
...
function! PGC_Workspace#Load_StoredWorkspace ( ... ) 
    if (a:0 == 0)
        echomsg "Empty args"
        return 
    endif
...
function! PGC_Workspace#VariablesComplete( ArgLead, CmdLine, CursorPos )
	return sort(map(filter(keys(g:), 'v:val =~# "\\V\\^PGCSession_' . (empty(a:ArgLead) ? '\\S' : escape(a:ArgLead, '\')) . '"'), 'v:val[11:]'))
endfunction

command! -bar -nargs=? -complete=customlist,PGC_Workspace#VariablesComplete WorkspaceSave call PGC_Workspace#Save_Workspace(<f-args>)
command! -bar -nargs=? -complete=customlist,PGC_Workspace#VariablesComplete WorkspaceLoad call PGC_Workspace#Load_StoredWorkspace(<f-args>)
